...
define $(PKG)_BUILD
	...
	cd '$(1)' && make -C byterun libcamlrun.a
	cd '$(1)' && make ocaml ocamlc
	cd '$(1)' && make -C stdlib
	cd '$(1)' && make -C tools ocamlmklib
	# Build ocamlopt
    cd '$(1)' && make opt
	...
	cd $(1) && make BINDIR=$(PREFIX)/$(TARGET)/bin \
	  LIBDIR=$(PREFIX)/$(TARGET)/lib/ocaml \
	  -C byterun install
	cd $(1) && make BINDIR=$(PREFIX)/$(TARGET)/bin \
	  LIBDIR=$(PREFIX)/$(TARGET)/lib/ocaml \
